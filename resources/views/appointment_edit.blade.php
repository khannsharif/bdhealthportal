              ...
              <form role="form" method="POST" @isset($editing) action="{{ route('update_appointment', ['id'=>$app->id]) }}" @else action="{{ url('/appointment/add') }}" @endif>
                  @csrf
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            ...
                            <input type="number" id="patient_id" name="patient_id" placeholder="" class="form-control" autofocus  required
                                   ...
                                   value="{{ old('patient_id', $app->patient_id) }}" >
                            @endhasanyrole
                            @hasanyrole('patient')
                        ...
                        <div class="col-sm-8">
                            <select name="department_id" class="form-control" id="department" required>
                                ...
                                @foreach($departments as $department)
                                    <option value="{{ $department->id }}" @if(old('department_id', $app->department_id)) selected @endif>{{ $department->department_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-8">
                            <select name="hospital_id" class="form-control" id="hospitals" required>
                                ...
                                @foreach($hospitals as $hospital)
                                    <option value="{{ $hospital->id }}" @if(old('hospital_id', $app->hospital_id)) selected @endif>{{ $hospital->hospital_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <input type="date" id="appointment_date" name="appointment_date" class="form-control" autofocus required value="{{old('appointment_date', $app->appointment_date)}}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="time_slot" class="col-sm-3 control-label">Available time slot</label>
                        <div class="col-sm-8">
                            <input type="time" id="time_slot" name="time_slot" class="form-control" autofocus required value="{{old('time_slot', \Carbon\Carbon::parse($app->appointment_time)->format('H:i:s'))}}">
                        </div>
                    </div>
                    {{--<div class="form-group row">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <textarea name="description" class="form-control"  placeholder="" rows="4" id="description"> {{old('description', $app->problem)}}</textarea>
                        </div>
                    </div>
                    <div class="form-group row">
